...
contract Web3Jobs {
    ...
    function getMyJobs(address employer) external view returns (bytes32[] memory) {
        return Employers[employer];
    }

    function getMyApplications(address applicant) external view returns (bytes32[] memory) {
        return MyApplications[applicant];
    }

    function canClaimBounty(address applicant, bytes32 jobId) external view returns (bool) {
        return
    ...
    function getMyApplicants(bytes32 jobId) external view returns (address[] memory) {
    ...
    function getAaveBalance() external view returns (uint256) {
        return 0; //IERC20(aWethTokenAddress).balanceOf(address(this));
    }
    ...
    function withdrawBounty(address token, uint256 amount) internal returns (bool) {
        // TODO: Implement withdraw from lending
        return true;
    }
